...
Future<void> markdown() async {
  ...
  var languageLength = filterMarkDown.length;

  if (languageLength == 0) ;
  {
    await ws?.values.insertColumn(2, ['0'], fromRow: 18);

    if (languageLength != 0) {
      await ws?.values.insertColumn(2, [languageLength], fromRow: 18);
    }
    ...
    if (filterMarkDown.isEmpty) {
      ...
      await ws?.values.insertColumn(
          3, ['${filterMarkDown.first.original.reference?.fragment?.string?.raw}'],
          fromRow: 18);
    }
  }
}
